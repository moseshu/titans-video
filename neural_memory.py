...
class NeuralMemory(Module):
        ...
        for (param_name, surprise), (_, last_update) in zip(surprises.items(), past_last_update.items()):
            ...
            if has_momentum:
                ...
                for one_adaptive_momentum, one_last_momentum in zip_longest(adaptive_momentum, last_momentum):
                    ...
                    orig_dtype = momentum.dtype
                    am_f32 = one_adaptive_momentum.float()
                    m_f32 = momentum.float()
                    prev_f32 = one_last_momentum.float() if exists(one_last_momentum) else None
                    momentum = self.assoc_scan(am_f32, m_f32, prev = prev_f32)
                    momentum = momentum.to(orig_dtype)
            ...
            gate = (1. - decay_factor).float()
            u_f32 = update.float()
            prev_u_f32 = last_update.float() if exists(last_update) else None
            update = self.assoc_scan(gate, u_f32, prev = prev_u_f32, remove_prev = False)
    ...
    def forward(
        ...
        if exists(self.transition_gate):
            ...
            gate = self.transition_gate[0].sigmoid()
